...
/**
 * SystemIntelRouter - Specialized router for information gathering and reconnaissance
 * Domain expert in environment discovery, target analysis, and capability assessment
 */
@Singleton
class SystemIntelRouter @Inject constructor(
    ...
    override val domain = RouterDomain.SYSTEM_INTELLIGENCE
    override val name = "SystemIntelRouter"
    ...
    override suspend fun executeStep(step: OperationStep, context: OperationContext): StepResult {
        ...
        return try {
            ...
            StepResult.failure(
                step.id,
                domain,
                "SystemIntel execution error: ${e.message}",
                executionTime
            )
        }
    }
